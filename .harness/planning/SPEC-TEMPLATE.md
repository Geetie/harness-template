...
## 设计决策（写代码前必答，四问）
> 这四问是**技术债最便宜的防线**：想清楚只要几分钟，事后重构要几小时。
> 答不上来就先别写代码 —— 答不上来通常意味着还没想清楚。

1. **单一职责**：本改动新增/修改的模块，各自"只有一个理由被改"是什么？
   - 答成"处理用户相关逻辑"= 没答（那是主题，不是职责）
   - 答成"校验注册入参"= 合格（一个理由：校验规则变了）
2. **依赖方向**：谁依赖谁？把依赖画成单向箭头，有没有环？
   - 出现双向依赖 → 立刻停，抽出第三方或改用事件/回调
3. **模式选择**：用了什么模式？为什么不用更简单的写法？
   - 答不上"为什么不用更简单" → 大概率过度设计（详见
     `.harness/design/DESIGN-PRINCIPLES.md` §模式选择的反向清单）
4. **扩展点**：预期哪类变化？现在的代码为它留口子了吗？
   - 预期不到的变化**不要**预留（YAGNI）—— 预留本身就是债
