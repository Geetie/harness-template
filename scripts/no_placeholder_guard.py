...
RULES: list[Rule] = [
    ...
    _r(
        # 判据校准（实测误报）：原来写 `raise(?!\s+\w*Error\s*\()`，只放过名字以
        # "Error" 结尾的异常构造 —— 于是 `raise Err(f"...{e}...") from e`
        # 这种**包装并加上下文**的正确写法被误判为"原样重抛"。
        # 改为：只要 raise 后跟的是「标识符 + 括号 + 括号内有内容」，就认为
        # 它在构造带消息的新异常（有上下文），不算占位式重抛。
        # 真正要抓的是裸 `raise` 和 `log; raise`（无上下文的透传）。
        "log-and-rethrow",
        "warn",
        r"(except[^\n:]*:\s*(?:#[^\n]*)?\n(?:\s+[^\n]*\n){0,3}?\s*raise(?!\s+\w+\s*\(\s*[^)\s])|"
